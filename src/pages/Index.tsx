      ...
      <section className="py-24 md:py-32 bg-gradient-section">
        <div className="container">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-heading font-bold mt-3">Fermerlar nima deydi?</h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {testimonials.map((t, i) => (
              <motion.div
                ...
                custom={i}
              >
                <Card className="h-full hover-lift border-0 shadow-lg shadow-primary/[0.03]">
                  <CardContent className="p-7 space-y-5">
                    <div className="flex gap-1">
                      {Array.from({ length: t.rating }).map((_, j) => (
                        <Star key={j} className="w-5 h-5 fill-sun text-sun" />
                      ))}
                    </div>
